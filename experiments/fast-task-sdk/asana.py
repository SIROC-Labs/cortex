...
# deterministic runner needs them as real verbs. Each is a single GET against the
# documented endpoint, projected to the minimum the caller needs.


# Shared shape for the three: GET <task>/<sub-path>, return the `data` list.
def _task_subresource(task_gid, sub_path, opt_fields):
    if not re.fullmatch(r"[0-9]+", task_gid):
        die(1, "%s: task-ref must be numeric (got '%s')" % (PROG, task_gid))
    key = cache_util.project_key()
    token = resolve_token(key)
    url = "%s/tasks/%s/%s?opt_fields=%s" % (API_BASE, task_gid, sub_path, opt_fields)
    payload = api_get(url, token)
    data = payload.get("data") if isinstance(payload, dict) else None
    return data if isinstance(data, list) else []


def task_subtasks(args):
    if not args:
        die(1, "usage: %s task subtasks <task-ref>" % PROG)
    rows = _task_subresource(args[0], "subtasks", "name,completed,gid")
    out = [{"ref": r.get("gid"), "name": r.get("name"),
...
# Dependencies are the tasks this one is BLOCKED BY. `completed` is what matters:
# an incomplete dependency means the task is not ready to start.
def task_dependencies(args):
    if not args:
        die(1, "usage: %s task dependencies <task-ref>" % PROG)
    rows = _task_subresource(args[0], "dependencies", "name,completed,gid")
    out = [{"ref": r.get("gid"), "name": r.get("name"),
            "completed": bool(r.get("completed"))}
           for r in rows if isinstance(r, dict)]
    sys.stdout.write(json.dumps(out, indent=2) + "\n")
...
# `download_url` is a pre-signed, short-lived S3 link — fetch it promptly or re-list.
# `is_image` drives caller behaviour: images are downloaded but not inlined as text.
IMAGE_EXT_RE = re.compile(r"\.(png|jpe?g|gif|webp|bmp|svg)$", re.IGNORECASE)


def task_attachments(args):
    if not args:
        die(1, "usage: %s task attachments <task-ref>" % PROG)
    rows = _task_subresource(
        args[0], "attachments", "name,download_url,resource_subtype,size")
    out = []
    for r in rows:
        ...
        out.append({
            ...
            "size": r.get("size"),
            "is_image": bool(IMAGE_EXT_RE.search(name)),
        })
    sys.stdout.write(json.dumps(out, indent=2) + "\n")
    sys.exit(0)
