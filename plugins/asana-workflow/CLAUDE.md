...
5. **Update the repo `README.md`** if the skill is user-invokable (has a slash command or responds to user trigger phrases directly) — add a row to the asana-workflow skill table. Internal helpers like `task-manager`, `task-manager-asana`, or `git-check` stay out of the user-facing table.
