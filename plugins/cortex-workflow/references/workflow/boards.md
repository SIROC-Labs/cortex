...
| `ready` | Reviewed and tested, not yet released | the operator |
...
**Dependencies.** A card waits while any card it depends on is neither completed nor at or past review. "At or past review" means the `in_review`, `ready` or `done` column of the agent board, or a column with the same name as one of them on any other board the blocker sits on. Anything else, including an unknown column or no board at all, blocks.
