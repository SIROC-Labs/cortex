...
- The `Refinement` and `Unassigned` lifecycle states (see `plugins/cortex-workflow/references/workflow/lifecycle.md`) are addressed by name through the `task-manager` interface — `set_status(task, "Unassigned")` later in Phase 3e. The provider resolves whatever underlying representation it uses; the skill never handles status identifiers itself.
...
Transition the task from `Refinement` to `Unassigned` via `set_status(task, "Unassigned")` (see `plugins/cortex-workflow/references/workflow/lifecycle.md`). The provider resolves the underlying representation.
