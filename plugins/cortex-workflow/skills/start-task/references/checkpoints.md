...
2. **Check for an existing checkpoint:**
   ```bash
   ls ~/.cortex/cortex-workflow/checkpoints/<task-ref>.md 2>/dev/null
   ```
...
4. **If no checkpoint** → create it via the helper:
   ...
   ```
   The script creates `~/.cortex/cortex-workflow/checkpoints/` if needed and writes the template with `task_ref`, `task_url`, `created_at`, `last_updated` pre-filled. No `.gitignore` entry is needed (the file lives outside the repo).
