...
- The target project must support the **Refinement** lifecycle state (see `plugins/cortex-workflow/references/workflow/lifecycle.md`). The skill verifies this before doing anything else and aborts with a clear message if it's missing.
...
2. **Learn which fields the board carries** via `list_fields(board)`. The canonical workflow fields (see `plugins/cortex-workflow/references/workflow/fields.md`) this skill sets are:
...
2. Set fields via `set_field(task, <field>, <value>)` for each field the board carries (see `plugins/cortex-workflow/references/workflow/fields.md`):
   ...
   - Priority — default `P3`
3. Set **Product Status** via `set_status(task, <name>)`, conditional on the task's Platform (lifecycle names from `plugins/cortex-workflow/references/workflow/lifecycle.md`):
