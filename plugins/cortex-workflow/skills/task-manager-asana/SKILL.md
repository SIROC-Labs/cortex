...
| `create_task(...)` | `scripts/tm.py task create <project-gid> --title T [--description D] [--assignee A] [--kind task\|milestone] [--set Name=Value ...] [--wait-key]` — `POST /tasks` (with `resource_subtype:"milestone"` when `--kind milestone`; no custom fields until added to a project) then `POST /tasks/<gid>/addProject`, then any `--set` fields in ONE batched `PUT` (same resolution as `set_field`), all via urllib (sandbox-safe). Prints the compact projection. The human key is assigned by an async Asana Rule; create does **not** block on it (resolved lazily by whoever needs it — create-pr re-fetches at PR time). Pass `--wait-key` only for a tight create-then-name-branch flow. Prefer `--set` over follow-up `set-field` calls — one PUT, fewer round-trips. |
