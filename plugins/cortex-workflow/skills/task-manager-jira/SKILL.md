...
| `list_tasks(board, column?)` | Active sprint → `acli jira sprint list-workitems --board <ID> --sprint <ID> --json --limit 200`; otherwise `acli jira workitem search --jql "…" --json --limit 200`. Each item carries `kind` (`milestone` for Milestone/Epic issue types, else `task`) (`references/boards.md`). With `column` (agent board): JQL `project = <key> [AND status = "<column>"] ORDER BY rank ASC` via `acli jira workitem search --json --limit 200`; project `{ref: key, name: summary, kind, completed: statusCategory == Done, assignee, fields}`. |
...
| `list_boards()` | `acli jira board search --json` → `[{ref: id, name, completed: false}]`. |
| `get_board(board)` | **PARTIAL until verified on a Jira instance** — the board's columns are its statuses. Use `acli jira board view --id <id> --json` when the installed `acli` offers it, mapping columns in configured order to `[{ref: <status id>, name: <status name>}]`; otherwise exit non-zero with the operator prompt to supply the column → status list, which the setup skill records as `column_names`. |
| `ensure_board(name, columns)` | **PARTIAL** — `acli` cannot create boards or columns. Exit non-zero with the operator prompt: create a board named `<name>` with columns `<columns>` in Jira, then re-run. |
| `ensure_columns(board, names)` | **PARTIAL** — same operator prompt naming the missing columns. |
| `move_task(task, board, column)` | Transition the issue to the column's status: list transitions, pick the one whose target status is the column, transition. |
| `get_dependencies(task)` | Issue links of the configured blocking type (inward "is blocked by"): per linked issue `{ref: key, name, completed: statusCategory == Done, memberships: [{board: {ref: <board id>, name}, column: {ref: <status id>, name: <status>}}]}`. |
