...
| `create_task(...)` | `scripts/tm.py task create <project-key> --title T [--description D] [--assignee A] [--kind task\|milestone] [--type T]` → `acli jira workitem create`. The neutral `kind` maps to a native issue **type**: `milestone` → the project's Milestone issue type (or Epic where none exists), `task` → `--type` (default `Task`). Priority / time tracking / sprint / custom fields are **not flags** → set them at creation via `acli ... --from-json` `additionalAttributes`, or post-create via the MCP fallback (`references/acli.md` → Create). |
